...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, features_folder = None, masks_folder = None):
    cam_infos = []
...
def readColmapSceneInfo(path, images, eval, llffhold=8, need_features=False, need_masks=False, replica=False):
    try:
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), features_folder=os.path.join(path, feature_dir) if need_features else None, masks_folder=os.path.join(path, mask_dir) if need_masks else None)
    if not replica:
